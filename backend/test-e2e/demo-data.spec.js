...
test.describe('four-role demo data walkthrough', () => {
  test('student and supervisor can demonstrate the complete review workflow', async ({ page }) => {
    ...
    await expect(page.getByText('[演示]student01-全文润色.txt')).toBeVisible();
    ...
    await expect(page.getByRole('link', { name: '查看详情' })).toHaveCount(2);
  ...
  test('college and school admins see scoped ledgers and populated management pages', async ({ page }) => {
    ...
    await page.goto('/');
    await expect(page.getByText('学院记录数').locator('..')).toContainText('9');
    await expect(page.getByText('覆盖学生数').locator('..')).toContainText('3');

    await page.goto('/quality-dashboard');
    await expect(page.getByText('样本数').locator('..')).toContainText('3');
    ...
    await page.goto('/');
    await expect(page.getByText('全校记录数').locator('..')).toContainText('12');
    await expect(page.getByText('覆盖学生数').locator('..')).toContainText('4');
    ...
    await page.goto('/quality-dashboard');
    await expect(page.getByText('样本数').locator('..')).toContainText('4');
  });
});

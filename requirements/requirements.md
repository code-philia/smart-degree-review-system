...
除明确标注的 review-pilot PDF 语义规则外，核心结果由本地可解释的规则、公式和持久化数据产生，不依赖 jAccount、短信、微信、外部大模型、校外文献库或未提供的校内论文库。语义规则固定调用 DeepSeek 官方 API，只处理规则选中的摘要、论点和候选论据文本，其结果属于需人工复核的辅助判断。
...
## MOD-UNIFIED-WORKBENCH 业务功能与历史一体化

基础规则检测、论文润色、创新性量表评估和规则化辅助评阅在侧边栏中各保留一个模块入口。当前演示版本将论文相似度检测及其历史页面保留为可直达的既有地址，但暂时不在侧边栏显示；原位置以“高级语义规则检测”替代，提供论点论据不一致、论文新颖性检测和相关对比工作推荐三个固定案例，分别使用独立 URL，不提供上传、运行或历史记录。用户进入有历史能力的模块后，通过同一页面区域内的页签切换“发起功能”和“历史记录”；论文润色提供“整篇润色”“局部润色”“润色记录”三个页签。报告详情可以保留独立 URL，切换到历史页或详情页时侧边栏仍高亮所属业务模块。
PDF 规则审查完成后保存为个人 PDF 检测报告，用户可在“历史报告”中继续查看检测结论、问题、原文定位和修改建议；原有纯文本规范检测记录仍保留并以独立类型展示。
Permissions: ALL.
### Included chapters
- `FEAT-UNIFIED-MODULE-HISTORY` 功能与历史模块页签

### FEAT-UNIFIED-MODULE-HISTORY 功能与历史模块页签

模块页签只改变前端信息架构，不放宽后端的数据权限和角色范围。历史记录仍按各业务原有规则加载，报告详情链接保持可刷新和可返回。
Permissions: ALL.
### Acceptance scenarios
#### 在同一业务模块切换发起功能和历史
Actor: STUDENT.
- **GIVEN (STUDENT)** student01 已登录并进入论文相似度检测。
- **WHEN (STUDENT)** 学生点击“历史记录”页签。
- **THEN** 系统仍将侧边栏“论文相似度检测”标记为当前模块，并仅显示 student01 的历史记录。
#### 合并论文润色入口
Actor: STUDENT.
- **GIVEN (STUDENT)** student01 已登录。
- **WHEN (STUDENT)** 学生从侧边栏进入“论文润色”。
- **THEN** 页面提供整篇润色、局部润色和润色记录三个页签，侧边栏不再重复展示三个独立入口。
...
## MOD-NORMATIVE-DETECTION 基础规则检测

模块保留原有纯文本规范检测与历史报告能力，同时在 `/normative-check` 提供 PDF 规则审查入口。PDF 入口把原文件临时交给同机规则引擎，返回可解释的规则状态、问题和页面坐标，并在浏览器中显示原 PDF 与高亮；成功结果和原文定位所需的 PDF 副本保存为仅本人可访问的个人报告。
PDF 入口默认启用 22 条不调用外部模型的确定性规则，覆盖中英文题名、摘要长度、关键词、正文标题、标题编号、首末章、目录格式、图表公式编号、参考文献格式一致性、正文图表引用完整性、中英文符号规范、目录正文一致性、核心章节完整性、页码连续性和引注编号连续性；另提供 3 条默认不勾选、需确认文本允许外发后才能运行的 DeepSeek 语义规则。参考文献格式一致性规则自动识别 GB/T 7714、APA 7 或 IEEE，识别证据不足或多种风格并列时明确报告无法稳定判断。正文图表引用完整性分别检查正文引用的图片、表格编号是否存在对应题注，不宣称已识别 PDF 中的真实视觉对象。原有纯文本规则、学校/学院规则发布及检测台账保持独立，不自动作用于 review-pilot 规则目录。
Permissions: ALL.
### Included chapters
...
所有已登录角色可在 `/normative-check` 上传不超过 50 MB 的 PDF，从部署环境提供的 review-pilot 规则目录中选择规则并同步运行。后端校验 PDF 文件头，以权限受限的临时文件调用同机 Python 规则引擎，限制并发数和单次运行时间，并在成功或失败后删除临时文件。
结果展示论文标题、规则执行状态和问题汇总；存在页面坐标的问题在内嵌 PDF 中高亮，点击问题或证据锚点时联动定位。系统默认启用 22 条不调用外部模型的确定性规则，并提供 3 条默认不勾选的语义规则：中英文摘要内容一致性、论点与论据一致性、强论断支撑检查。确定性规则中的参考文献格式一致性检查自动判断 GB/T 7714、APA 7 或 IEEE，并将不同于全文主风格或无法识别的条目定位为问题；图片和表格引用完整性规则分别核对正文引用编号与全文对应题注。语义规则固定使用 DeepSeek 官方 `deepseek-v4-flash` 非思考模式，不提供模型选择或应用层重试；运行前明确提示相关摘要、论点和候选论据文本会发送至外部 API，用户不得上传不允许外发的论文。模型密钥未配置时语义规则显示为不可用。
原始 PDF 不写入 SQLite 二进制字段；系统在应用专用目录保存原文定位所需的 PDF 副本，并仅通过后端登录权限向报告创建者提供。PDF 报告进入个人检测历史，但不自动进入现有文本检测台账、统计或师生批阅闭环。语义模型输出可能不稳定或无法判定，不得表述为专家结论，必须由人工复核。扫描版、加密 PDF 或缺少规则所需结构时，规则可返回不适用、无法判定或可理解的错误。
Permissions: ALL.
Depends on: `FEAT-AUTH-SESSION`.
### Acceptance scenarios
#### 运行 PDF 规则并联动定位
...
#### 自动判断参考文献格式并报告混用
Actor: STUDENT.
- **GIVEN (STUDENT)** PDF 的大多数参考文献符合 GB/T 7714，另有一条采用 APA 7。
- **WHEN (STUDENT)** 学生运行参考文献格式一致性规则。
- **THEN** 系统将全文主格式识别为 GB/T 7714，并定位报告采用 APA 7 的条目；风格证据不足或并列时不伪造主格式。
#### 分别报告正文引用但无对应题注的图表
Actor: STUDENT.
- **GIVEN (STUDENT)** 正文引用“图 2”和“表 4”，全文仅存在“图 1”和“表 3”的题注。
- **WHEN (STUDENT)** 学生运行图片和表格引用完整性规则。
- **THEN** 系统分别定位“图 2”和“表 4”的正文引用并提示未识别到对应题注，不把该结果表述为视觉对象检测结论。
#### 拒绝无效输入且不保留临时文件
Actor: STUDENT.
...
#### 按需运行语义规则并说明外部处理边界
Actor: STUDENT.
- **GIVEN (STUDENT)** DeepSeek 官方 API Key 已由服务器安全配置，学生选择至少一条标记为“DeepSeek 语义”的规则。
- **WHEN (STUDENT)** 学生确认论文允许外发并发起审查。
- **THEN** 系统只把该规则选中的摘要、论点和候选论据文本发送至固定模型，返回真实的通过、问题、无法判定或执行失败状态，并提示结果需要人工复核。
#### 密钥未配置时不伪造语义结果
Actor: SCHOOL_ADMIN.
...
所有已登录角色可从独立的“内置审查案例”模块查看随系统版本提供的固定 PDF 与确定性案例结果。案例包括同一论文中 Yandex 准确率 `80.0%` 的论点与实验章节 `60.5%` 的论据冲突，以及中文摘要遥感影像分割与英文摘要验证码识别的研究内容不一致；结果包含修改建议和 PDF 原文坐标。案例不写入 SQLite，不进入检测历史、统计或师生批阅闭环。
Permissions: ALL.
...
Actor: STUDENT.
- **GIVEN (STUDENT)** student01 已登录并进入内置审查案例模块。
- **WHEN (STUDENT)** 学生打开“中英文摘要研究内容不一致”案例，并分别点击中文摘要和英文摘要锚点。
- **THEN** 系统展示固定的一项摘要内容不一致问题，并将 PDF 定位到第 7 页中文摘要与第 9 页英文摘要。
#### 拒绝匿名读取案例资源
Actor: STUDENT.
- **GIVEN (STUDENT)** 浏览器不存在有效会话。
...
学生可粘贴论文文本或上传 .txt/.md/含可搜索文字层的 PDF 文件，选择当前生效的学校/学院规则后发起检测。
...
学校管理人员可通过粘贴文本或上传 .txt/.md/含可搜索文字层的 PDF 文件新增样本，填写标题、学科和年份，并可查看或删除样本。
...
### FEAT-DUPLICATION-DETECT 发起校内库查重与 AIGC 写作风险检测

页面提供“校内库查重”和“AIGC 写作风险检测”两类选择。“校内库查重”对文本进行小写化、空白归一化和标点移除，以字符 5-gram 集合计算 Jaccard 相似度，返回最高的五个样本及相似段落。总相似率为所有命中段落字符数去重后除以待检文本有效字符数，比对阈值默认为 0.65。
“AIGC 写作风险检测”不访问样本库，风险分由段落重复率 35%、句长变化不足 25%、模板连接词密度 20%和空泛短语密度 20% 加权生成。页面必须显示该分数是启发式风险而非 AI 真伪结论。
...
支持粘贴文本或上传 .txt/.md/含可搜索文字层的 PDF，提供三档可解释处理：
...
用户选择评阅模板，填写论文题目，并粘贴或上传 .txt/.md/含可搜索文字层的 PDF 论文文本。系统按标题行和关键词识别章节，统计字符数与参考文献条目，复用规范检测规则，并按模板公式生成结果。

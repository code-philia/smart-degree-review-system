...
PDF 快速规则审查仍不保存结果，页面必须在“发起审查/历史报告”页签附近明确说明现有历史仅包含系统已持久化的规范检测记录。
Permissions: ALL.
### Included chapters
...
原始 PDF 和本次快速审查结果不写入当前 SQLite，处理完成后删除后端临时文件，不进入现有检测历史、统计和师生批阅闭环。语义模型输出可能不稳定或无法判定，不得表述为专家结论，必须由人工复核。扫描版、加密 PDF 或缺少规则所需结构时，规则可返回不适用、无法判定或可理解的错误。
Permissions: ALL.
Depends on: `FEAT-AUTH-SESSION`.
### Acceptance scenarios
#### 运行 PDF 规则并联动定位

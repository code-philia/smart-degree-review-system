...
对纯文本论文执行可解释、可重复的规则检测。系统支持 .txt、.md、含可搜索文字层的 PDF 文件或直接粘贴文本，单份文件不超过 5 MB，PDF 不超过 500 页。PDF 在浏览器中提取为纯文本后进入现有检测链路，不保存原始 PDF。
...
#### 拒绝超限或不可提取文件
Actor: STUDENT.
- **GIVEN (STUDENT)** 文件超过 5 MB、文本文件无法按 UTF-8 解码，或 PDF 不含可搜索文字层。
- **WHEN (STUDENT)** 学生上传文件。

...
PDF 入口默认启用 16 条不调用外部模型的确定性规则，覆盖中英文题名、摘要长度、关键词、正文标题、标题编号、首末章、目录格式、图表公式编号、参考文献基础著录和中英文符号规范；另提供 3 条默认不勾选、需确认文本允许外发后才能运行的 DeepSeek 语义规则。原有纯文本规则、学校/学院规则发布及检测台账保持独立，不自动作用于 review-pilot 规则目录。
Permissions: ALL.
### Included chapters
...
结果展示论文标题、规则执行状态和问题汇总；存在页面坐标的问题在内嵌 PDF 中高亮，点击问题或证据锚点时联动定位。系统默认启用 16 条不调用外部模型的确定性规则，并提供 3 条默认不勾选的语义规则：中英文摘要内容一致性、论点与论据一致性、强论断支撑检查。语义规则固定使用 DeepSeek 官方 `deepseek-v4-flash` 非思考模式，不提供模型选择或应用层重试；运行前明确提示相关摘要、论点和候选论据文本会发送至外部 API，用户不得上传不允许外发的论文。模型密钥未配置时语义规则显示为不可用。

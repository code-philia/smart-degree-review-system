...
function ReviewRubricSelector({ isOpen }: ReviewRubricSelectorProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      Promise.allSettled([fetchReviewRubrics(), new Promise((resolve) => window.setTimeout(resolve, 80))]).then(
        ([rubricResult]) => {
          if (!mounted) {
            return;
          }
        ...
        {rubrics.templates.map((template) => (
          <article key={template.template_id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-sm font-semibold text-blue-700">
              最低参考文献数量：{template.minimum_reference_count}
            </p>
      ...
      <section className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-sm font-semibold">
          <span className="rounded-full bg-emerald-100 px-3 py-1 text-emerald-700">
            {rubrics.passing_rule.pass_label}
          </span>
          <span className="rounded-full bg-amber-100 px-3 py-1 text-amber-700">
            {rubrics.passing_rule.revise_label}
          </span>
        </div>
        <p className="mt-3 text-sm font-semibold text-slate-700">
          客观分不低于 {rubrics.passing_rule.minimum_objective_score} 且无必需章节缺失时为“
          {rubrics.passing_rule.pass_label}”，否则为“{rubrics.passing_rule.revise_label}”。
        </p>
      </section>
    </div>
  );

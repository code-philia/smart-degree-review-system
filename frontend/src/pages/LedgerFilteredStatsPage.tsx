    ...
    <div>
      ...
      <section className="flex flex-wrap items-center justify-between gap-3 pb-3">
        <div className="flex flex-wrap gap-3">
          <button className="h-11 rounded-sm border border-[#3b86ee] bg-[#3b86ee] px-8 text-[17px] font-semibold text-white">
            筛选统计
          </button>
          <button className="h-11 rounded-sm border border-[#d6d6d6] bg-white px-8 text-[17px] font-semibold text-[#303b45]">
            每日趋势
          </button>
        </div>
        <p className="text-[#536476]">
          当前筛选记录数：<strong className="text-[#1f3f63]">{stats?.total_records ?? 0}</strong>
        </p>
        <button className="h-11 bg-[#46c33f] px-10 font-bold text-white" type="button">
          导出报表
        </button>
      </section>
          ...
          <label className="grid gap-2 text-[15px] font-semibold text-[#1f3f63]">
            ...
            <select
              ...
              onChange={(event) =>
                setFilters((current) => ({
                  ...current,
                  detection_type: event.target.value as DetectionLedgerType | '',
                }))
              }
            >
      ...
      <section className="grid gap-4 py-5 md:grid-cols-4">
        <article className="rounded-sm border border-[#d6d6d6] bg-white p-5">
          <p className="text-[#536476]">记录数</p>
          <strong className="text-[30px] text-[#1f3f63]">{stats?.total_records ?? 0}</strong>
        </article>
        <article className="rounded-sm border border-[#d6d6d6] bg-white p-5">
          <p className="text-[#536476]">参与学生数</p>
          <strong className="text-[30px] text-[#1f3f63]">{stats?.total_students ?? 0}</strong>
        </article>
        <article className="rounded-sm border border-[#d6d6d6] bg-white p-5">
          <p className="text-[#536476]">今日数量</p>
          <strong className="text-[30px] text-[#1f3f63]">{stats?.today_count ?? 0}</strong>
        </article>
        <article className="rounded-sm border border-[#d6d6d6] bg-white p-5">
          <p className="text-[#536476]">当前类型</p>
          <strong className="text-[22px] text-[#1f3f63]">{activeTypeLabel}</strong>
        </article>
      </section>
      ...
      <section className="grid gap-5 pb-8 lg:grid-cols-2">
        <article className="min-h-[280px] rounded-sm border border-[#d6d6d6] bg-white p-5">
          ...
          {stats && stats.by_type.length > 0 && (
            <div className="flex h-44 items-end gap-4 border-l border-b border-[#d6d6d6] px-4">
              {stats.by_type.map((item) => (
                <div key={item.detection_type} className="flex flex-1 flex-col items-center gap-2">
                  <div
                    className="w-full bg-[#3b86ee]"
                    style={{ height: `${Math.max(item.record_count || item.total_records || 0, 1) * 8}px` }}
                  />
                  <span className="text-xs text-[#536476]">{item.detection_type_label}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <article className="min-h-[280px] rounded-sm border border-[#d6d6d6] bg-white p-5">
          ...
          {stats && stats.daily_trend.length > 0 ? (
            <svg className="h-44 w-full border-l border-b border-[#d6d6d6]" role="img" aria-label="每日趋势折线图">
              <polyline
                fill="none"
                stroke="#3b86ee"
                strokeWidth="3"
                points={stats.daily_trend
                  .map(
                    (point, index) =>
                      `${20 + index * 60},${150 - Math.min(point.count || point.record_count || point.total_records || 0, 10) * 12}`,
                  )
                  .join(' ')}
              />
            </svg>

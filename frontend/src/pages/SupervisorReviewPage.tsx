  ...
  return (
    <div className="text-slate-900">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <Link className="text-xs font-semibold text-brand-600 hover:underline" to="/supervisor-review-queue">
            待批阅任务
          </Link>
          <h1 className="mt-1 text-lg font-bold text-[#173a5e]">原报告批注与整体反馈</h1>
        </div>
        <Link
          className="rounded border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100"
        ...
        <section className="grid gap-4 lg:grid-cols-[3fr_2fr]">
              ...
              {groupedFindings.length === 0 ? (
                ...
                groupedFindings.map(([category, findings]) => (
                  <div key={category} className="mb-4 overflow-hidden rounded border border-slate-200">
                    <div className="bg-[#e9f7ff] px-3 py-2 font-bold text-[#1f436a]">
          ...
          <aside className="rounded border border-slate-200 bg-white p-4 text-sm">
            <div className="mb-3 flex items-center justify-between">
              ...
              {locked ? (
                <span className="rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-700">已锁定</span>
              ) : null}
            </div>
            <div className="mb-4 rounded border border-dashed border-blue-200 bg-[#fffbea] p-3">
              {annotations.length === 0 ? (
                <p className="text-slate-500">尚未添加问题批注。</p>
              ) : (
                annotations.map((annotation, index) => (
                  <div
                    key={`${annotation.finding_id}-${index}`}
                    className="border-b border-yellow-100 py-2 last:border-b-0"
                  >
            ...
            <button
              className="mb-4 rounded border border-blue-500 px-3 py-2 font-bold text-blue-600 disabled:opacity-50"
            ...
            <label className="mb-3 block font-semibold text-green-700">
              整体评价（必填）
              <textarea
                className="mt-1 min-h-24 w-full rounded border border-slate-300 px-3 py-2 text-slate-900"
                disabled={locked}
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                className="rounded bg-[#2f80ed] px-5 py-2 font-bold text-white disabled:opacity-50"

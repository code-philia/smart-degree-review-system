  ...
  return (
    <div className="text-[#1f2d3d]">
        ...
        <div className="grid gap-4 bg-[#f8fafc] p-5 md:grid-cols-4">
          <label className="grid gap-2 text-[15px] font-semibold text-[#1f3f63]">
            ...
            <span className="flex items-center gap-2">
              <input
                className="h-11 w-full border border-[#d6d6d6] bg-white px-3 font-normal"
                type="date"
                value={filters.from || ''}
          ...
          <label className="grid gap-2 text-[15px] font-semibold text-[#1f3f63]">
            ...
            <select
              ...
              onChange={(event) =>
                setFilters((current) => ({
                  ...current,
                  detection_type: event.target.value as LedgerRecordFilters['detection_type'],
                }))
              }
            >
          ...
          <div className="flex items-end gap-3">
            <button
              className="h-11 border border-[#d6d6d6] bg-white px-8 font-bold text-[#303b45]"
              type="button"
              onClick={() => {
      ...
      {status === 'idle' && !dashboard && (
        <p className="py-16 text-center text-[#536476]">请选择筛选条件后生成群体质量仪表盘</p>
      )}
        ...
        <>
          <section className="grid gap-4 px-7 pb-5 md:grid-cols-5">
            <article className="rounded-sm border border-[#d6d6d6] bg-white p-5">
              <p className="text-[#536476]">样本数</p>
              <strong className="text-[30px] text-[#1f3f63]">{dashboard.sample_count}</strong>
            </article>
            {dashboard.metrics.map((metric) => (
              <article key={metric.key} className="rounded-sm border border-[#d6d6d6] bg-white p-5">
                <p className="text-[#536476]">{metric.label}</p>
                <strong className="text-[26px] text-[#1f3f63]">{formatScore(metric.average_score)}</strong>
                <p className="mt-2 text-sm text-[#536476]">
                  有效 {metric.sample_count}，缺失 {metric.missing_count}
                </p>
              </article>
            ))}
          </section>
          ...
          <section className="grid gap-5 px-7 pb-8 lg:grid-cols-2">
            <article className="rounded-sm border border-[#d6d6d6] bg-white p-5">
              ...
              {dashboard.metrics.every((metric) => metric.distribution.length === 0) ? (
                <p className="py-16 text-center text-[#536476]">暂无分布数据</p>
              ) : (
                dashboard.metrics.map((metric) => (
                  <div key={metric.key} className="mb-4">
                    <p className="mb-2 font-semibold text-[#1f3f63]">{metric.label}</p>
                    <div className="flex h-20 items-end gap-2 border-l border-b border-[#d6d6d6] px-3">
                      {metric.distribution.map((bucket) => (
                        <div key={bucket.range} className="flex flex-1 flex-col items-center gap-1">
                          <div
                            className="w-full bg-[#3b86ee]"
                            style={{ height: `${Math.max(bucket.count, 1) * 8}px` }}
                          />
                          <span className="text-xs text-[#536476]">{bucket.range}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <article className="rounded-sm border border-[#d6d6d6] bg-white p-5">
              ...
              {dashboard.students.length === 0 ? (
                <p className="py-16 text-center text-[#536476]">当前筛选条件下暂无学生质量数据</p>
              ) : (
                <div className="overflow-auto">
                  <table className="w-full min-w-[640px] border-collapse text-left">
                    <thead>
                      <tr className="bg-[#f8fafc] text-[#1f3f63]">
                        <th className="border border-[#d6d6d6] p-3">学生</th>
                        <th className="border border-[#d6d6d6] p-3">规范分</th>
                        <th className="border border-[#d6d6d6] p-3">原创参考分</th>
                        <th className="border border-[#d6d6d6] p-3">创新参考分</th>
                        <th className="border border-[#d6d6d6] p-3">评阅基础分</th>
                      </tr>
                    </thead>
                    <tbody>
                      {dashboard.students.map((student) => (
                        <tr key={student.student_id}>
                          <td className="border border-[#d6d6d6] p-3">{student.student_name}</td>
                          <td className="border border-[#d6d6d6] p-3">{formatScore(student.scores.normative)}</td>
                          <td className="border border-[#d6d6d6] p-3">{formatScore(student.scores.originality)}</td>
                          <td className="border border-[#d6d6d6] p-3">{formatScore(student.scores.innovation)}</td>
                          <td className="border border-[#d6d6d6] p-3">{formatScore(student.scores.review_base)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
